...
const Applications = () => {
  ...
  const handleRevokeApp = async (appId) => {
    if (!window.confirm("Are you sure you want to revoke this application? All associated API access will be immediately terminated.")) {
      return;
    }
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Connected Applications</h1>
          <p className="text-gray-500 text-sm font-medium mt-1">Manage API integrations and secure access keys</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {apps.length > 0 ? apps.map((app) => (
          <div key={app.id} className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all group border-b-4 border-b-transparent hover:border-b-primary-600">
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-gray-400 uppercase font-black mt-0.5 tracking-widest">Active Application</p>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="relative group/key">
                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest block mb-1.5">API Identity Key</span>
                <div className="bg-gray-50 border border-gray-100 rounded-lg px-3 py-2 flex items-center justify-between gap-3 group-hover/key:bg-white transition-colors">
                   <code className="text-[10px] font-mono text-primary-600 font-bold truncate">{app.api_key}</code>
                   <button 
                    onClick={() => copyToClipboard(app.api_key, `${app.id}-key`)}
                    className="shrink-0 p-1 hover:bg-gray-100 rounded transition-colors text-gray-400"
                   >
                     {copiedKey === `${app.id}-key` ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
                   </button>
                </div>
              </div>
          ...
          <div className="col-span-full py-20 text-center bg-gray-50/50 border border-dashed border-gray-200 rounded-3xl">
             <div className="flex flex-col items-center gap-4">
                ...
                <div>
                   <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wide">Infrastructure Empty</h3>
                   <p className="text-xs text-gray-400 font-medium mt-1">Create your first application lifecycle monitoring endpoint.</p>
                </div>
             </div>
          </div>
        )}
      ...
      {revealModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-sm animate-in fade-in duration-300" onClick={() => setRevealModal(null)}></div>
          <div className="bg-white rounded-3xl w-full max-w-xl relative overflow-hidden shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
            <div className="bg-emerald-600 p-8 text-white">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => setRevealModal(null)} className="p-2 hover:bg-white/10 rounded-xl transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <p className="text-emerald-50 text-sm font-medium leading-relaxed opacity-90">
                This secret will only be shown <span className="underline decoration-2 underline-offset-4">ONCE</span>. Please store it immediately in a secure vault. You will not be able to retrieve it again.
              </p>
            </div>
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1 text-emerald-600">Secure API Secret</label>
                <div className="flex items-center gap-3">
                  ...
                  <button 
                    onClick={() => copyToClipboard(revealModal.api_secret, 'reveal-secret')}
                    className="shrink-0 p-3 bg-white border border-emerald-100 rounded-xl hover:bg-emerald-50 transition-all text-emerald-600"
                  >
                    {copiedKey === 'reveal-secret' ? <Check className="w-5 h-5" /> : <Copy className="w-5 h-5" />}
                  </button>
                </div>
              </div>
      ...
      <div className="bg-emerald-600 rounded-2xl p-8 text-white relative overflow-hidden shadow-lg shadow-emerald-600/20">
         <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="max-w-xl">
               <h3 className="text-xl font-bold mb-2">Enterprise Security Notice</h3>
               <p className="text-emerald-50 text-sm opacity-90 leading-relaxed font-medium">
                  API secrets are one-time visible entities. Once an application is established, you must store your secret in a hardware vault or secure environment configuration.
               </p>
            </div>
