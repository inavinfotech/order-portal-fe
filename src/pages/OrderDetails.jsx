...
const OrderDetails = () => {
  ...
  const fetchDetails = useCallback(async () => {
    try {
      ...
      const valid = transRes.data.filter(t => t.from_state === orderRes.data.status);
  ...
  const handleTerminate = async () => {
    if (!window.confirm("Are you sure you want to terminate this sequence? This action is irreversible in the standard workflow.")) {
      return;
    }
  ...
  const handleTransition = async (toStatus, force = false) => {
    if (force && !window.confirm(`Warning: This is a non-standard transition. Are you sure you want to force the state change to "${toStatus}"?`)) {
      return;
    }
    ...
    try {
      await api.post(`/orders/${id}/transition`, { 
        to_status: toStatus, 
        notes: force ? `Forced override via Portal UI` : `Transition via Portal UI`,
        force: force
      });
      await fetchDetails();
    } catch (err) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500 max-w-5xl mx-auto print:space-y-4">
      <div className="flex items-center justify-between print:hidden">
        <Link to="/orders" className="flex items-center gap-2 text-gray-500 hover:text-gray-900 transition-colors group">
          <ArrowLeft className="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" />
          <span className="text-[10px] font-bold uppercase tracking-widest">Back to Registry</span>
        </Link>
        <div className="flex gap-2">
          <button onClick={() => window.print()} className="bg-white hover:bg-gray-50 text-gray-700 text-[10px] font-bold py-2 px-4 rounded-lg transition-all border border-gray-200 uppercase tracking-tight">
            Print Records
          </button>
          <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-gray-200 rounded-xl p-8 shadow-sm">
             <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                <div className="space-y-1.5">
                  <div className="flex items-center gap-3">
                    <h1 className="text-xl font-bold text-gray-900 uppercase tracking-tight">{order.product_name}</h1>
                      ...
                      <div className="relative inline-block print:hidden">
                        <select 
                          ...
                          onChange={(e) => {
                            ...
                            const isValid = transitions.some(t => t.to_state === newStatus);
                  ...
                  <p className="text-gray-400 font-mono text-[10px] uppercase">UUID: {order.id}</p>
                </div>
                <div className="text-left md:text-right">
                   <p className="text-2xl font-black text-gray-900">{currencySymbol}{order.total_amount.toFixed(2)}</p>
                   <p className="text-[10px] text-gray-400 font-bold tracking-widest uppercase mt-0.5">Grand Total ({order.currency})</p>
                </div>
             </div>
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-5 bg-gray-50 rounded-xl border border-gray-100">
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs font-bold text-gray-900">{new Date(order.created_at).toLocaleDateString()}</p>
                </div>
                <div className="space-y-0.5">
                  <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                    <Package className="w-3 h-3 text-gray-300" /> Units
                  </p>
                  <p className="text-xs font-bold text-gray-900">{order.quantity} Item(s)</p>
                </div>
                <div className="space-y-0.5">
                  <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                    <CreditCard className="w-3 h-3 text-gray-300" /> Net Total
                  </p>
                  <p className="text-xs font-bold text-gray-900 uppercase font-mono">{order.currency}</p>
                </div>
             </div>
          </div>
                 ...
                 <table className="w-full text-left border-collapse">
                   <thead>
                     <tr className="border-b border-gray-100 bg-gray-50/50">
                       <th className="py-3 px-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Product ID</th>
                       <th className="py-3 px-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-center">Qty</th>
                   ...
                   <tbody className="divide-y divide-gray-50">
                     {order.items.map((item, idx) => (
                       <tr key={item.id || idx} className="hover:bg-gray-50/50 transition-colors">
                         <td className="py-3 px-4 text-xs font-bold text-gray-900">{item.product_id}</td>
                         <td className="py-3 px-4 text-xs font-medium text-gray-600 text-center">{item.quantity}</td>
                         <td className="py-3 px-4 text-xs font-medium text-gray-600 font-mono text-right">{currencySymbol}{item.unit_price.toFixed(2)}</td>
                         <td className="py-3 px-4 text-xs font-black text-gray-900 font-mono text-right">{currencySymbol}{(item.quantity * item.unit_price).toFixed(2)}</td>
                       </tr>
                     ))}
                   </tbody>
                 </table>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-8 shadow-sm">
             ...
             <div className="relative space-y-8 ml-2">
                ...
                {order.history && order.history.length > 0 ? [...order.history].reverse().map((entry, idx) => (
                  <div key={idx} className="relative flex gap-5 pl-8 items-start group">
                    ...
                    <div className="space-y-1.5 pb-6 border-b border-gray-50 flex-1">
                       <div className="flex items-center justify-between">
                          <p className="text-xs font-bold text-gray-900">
                             Transition: <span className="text-gray-400 font-medium uppercase px-1">{entry.from_status}</span> 
                             <ArrowRight className="inline w-3 h-3 text-gray-300 mx-1" />
                             <span className="text-primary-700 font-black uppercase px-1">{entry.to_status}</span>
                          </p>
                          <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">
                             {new Date(entry.changed_at).toLocaleString()}
                          </span>
                       </div>
                       <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight italic">Initiated by {entry.changed_by}</p>
                       {entry.notes && (
                  ...
                  <div className="text-center py-10">
                     <p className="text-gray-400 text-xs font-bold uppercase tracking-widest italic">Initial Sequence Start</p>
                  </div>
                )}
                ...
                <div className="relative flex gap-5 pl-8 items-start group">
                  ...
                  <div className="space-y-0.5">
                    <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest">ORDER CREATED</p>
        ...
        <div className="space-y-6">
           ...
           <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <h3 className="text-gray-900 text-xs font-bold mb-5 uppercase tracking-wide">System Metadata</h3>
              <div className="space-y-4">
                 <div className="flex justify-between items-center text-[10px]">
                    <span className="text-gray-400 font-bold uppercase tracking-widest">Application ID</span>
                    <span className="text-gray-900 font-black uppercase">{order.application_id.slice(0, 8)}</span>
                 </div>
                 <div className="flex justify-between items-center text-[10px]">
                    <span className="text-gray-400 font-bold uppercase tracking-widest">User Handle</span>
                    <span className="text-gray-900 font-black uppercase">UID-{order.user_id}</span>
                 </div>
                 <div className="flex justify-between items-center text-[10px]">
                    <span className="text-gray-400 font-bold uppercase tracking-widest">Compliance</span>
                    <span className="text-emerald-600 font-black uppercase tracking-tighter">Verified</span>
                 </div>
              </div>
           </div>
           ...
           <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex flex-col items-center text-center print:hidden">
              ...
              <h4 className="text-gray-900 text-xs font-bold mb-2 uppercase tracking-tight">Sequence Support</h4>
              <p className="text-[10px] text-gray-500 leading-relaxed mb-6 font-medium">
                 Encountering anomalies with this specific sequence? Open an enterprise support ticket.
              </p>
