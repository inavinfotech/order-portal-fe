...
const Dashboard = () => {
  ...
  const statCards = [
    ...
    {
      name: "Total Revenue",
      value: `${currencySymbol}${stats.amount.toLocaleString()}`,
      icon: DollarSign,
    ...
    },
    {
      ...
      value: stats.growth,
      icon: TrendingUp,
      color: stats.growth.startsWith("+")
        ? "text-emerald-600"
        : "text-primary-600",
      bg: stats.growth.startsWith("+") ? "bg-emerald-50" : "bg-primary-50",
    },
  ];
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div>
        ...
        <p className="text-gray-500 text-sm font-medium mt-1">
          Real-time fulfillment metrics and performance
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((card) => (
          <div
            key={card.name}
            className="bg-white border border-gray-200 p-6 rounded-xl group hover:border-primary-500/50 transition-all duration-300 shadow-sm hover:shadow-md"
          >
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[10px] font-bold ${health.api === "online" ? "text-emerald-600" : "text-rose-600"} flex items-center gap-1.5 uppercase transition-all`}
                >
                  <span
                    className={`w-1.5 h-1.5 ${health.api === "online" ? "bg-emerald-500" : "bg-rose-500"} rounded-full ${health.api === "online" ? "animate-pulse" : ""}`}
                  ></span>
                  {health.api}
                </span>
              </div>
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[10px] font-bold ${health.database === "online" ? "text-emerald-600" : "text-rose-600"} flex items-center gap-1.5 uppercase`}
                >
                  <span
                    className={`w-1.5 h-1.5 ${health.database === "online" ? "bg-emerald-500" : "bg-rose-500"} rounded-full ${health.database === "online" ? "animate-pulse" : ""}`}
                  ></span>
                  {health.database}
                </span>
              </div>
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[10px] font-bold ${health.relay === "online" ? "text-emerald-600" : "text-rose-600"} flex items-center gap-1.5 uppercase`}
                >
                  <span
                    className={`w-1.5 h-1.5 ${health.relay === "online" ? "bg-emerald-500" : "bg-rose-500"} rounded-full ${health.relay === "online" ? "animate-pulse" : ""}`}
                  ></span>
                  {health.relay}
                </span>
              </div>
